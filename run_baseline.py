...
def oneline_wrapper(dic_exp_conf, dic_agent_conf, dic_traffic_env_conf, dic_path):
    oneline = OneLine(dic_exp_conf=merge(config.DIC_EXP_CONF, dic_exp_conf),
                      dic_agent_conf=merge(
                          getattr(config, "DIC_{0}_AGENT_CONF".format(dic_exp_conf["MODEL_NAME"].upper())),
                          dic_agent_conf),
...
def main(memo, rall, road_net, env, gui, volume, ratio, model, count, lane):
    ...
    if rall:
        traffic_file_list = [ENVIRONMENT + "_" + road_net + "_%d_%s" % (v, r) for v in [300, 500, 700] for r in
                             [0.3, 0.6]]
    else:
...
if __name__ == "__main__":
    ...
    # memo = "test"
    # args.g = True
    main(args.memo, args.all, args.road_net, args.env, args.gui, args.volume, args.ratio, args.model, args.count,
         args.lane)
    end = time.time()
    duration = end - start
